...
if (!source || !output) throw new Error("Usage: node scripts/import_intent_catalog.mjs <source.xlsx> <output.json> [sheet]");
...
for (const row of rows.slice(1)) {
  ...
  products.push({
    id: `intent-camping-${itemId}`, title, category: category || xcat2, xcat1, xcat2,
    ...
    attributes: [...new Set(["露营", xcat1, xcat2, commodity, category].filter(Boolean))],
    baseScore: ordercost, novelty: 0, brand: "淘宝", origin: "",
    audiences: [], styles: [], goals: ["露营"], trend: 0,
  });
}
await fs.mkdir(path.dirname(output), { recursive: true });
await fs.writeFile(output, `${JSON.stringify({ scene: "camping", triggers: ["露营", "野营", "野炊"], products })}\n`);
console.log(`Imported ${products.length} camping intent products.`);
